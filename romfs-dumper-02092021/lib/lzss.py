...
class lzss():
    ...
    def InsertNode(self, i):
        ...
        parent_index = i
        parent_link =  (self.context["ring_buffer_size"] + 1) + keyi + keyii
        ...
        while True:
            ...
            if cmp >= 0:
                ...
                cmp = self.context["lchild"][parent_link]
                matched_list = self.context["lchild"]

            if cmp == self.context["root_index"]:
                parent_index = i
                child_index = parent_link
                child_link = i
                break
            ...
            while ii < self.context["max_match_length"]:
                cmp = self.context["ring_buffer"][i + ii] - self.context["ring_buffer"][parent_link + ii]
    ...
    def Lzss_Compress(self, uncompressed_data):
        ...
        i = 0
        ring_index = 0
        ring_footer_start = self.context["ring_buffer_size"] - self.context["max_match_length"]
        footer_index = ring_footer_start

        length = 0
        while length < self.context["max_match_length"] and i < uncompressed_size:
            self.context["ring_buffer"][footer_index + length] = uncompressed_data[i]
        ...
        while length > 0:
            ...
            if self.context["match_length"] < self.context["match_threshold"]:
                ...
                code_buffer[0] = (code_buffer[0] | mask.value) & 0xFF
                code_buffer[code_buffer_index] = self.context["ring_buffer"][footer_index] & 0xFF
                code_buffer_index += 1
                code_add += 1
            else:
                match_position = footer_index - self.context["match_position"]
                if (match_position - 1) >= 0:
                    match_position -= 1
                else:
                    match_position += (self.context["ring_buffer_size"] - 1)

                code_buffer[code_buffer_index] = match_position & 0xFF
                code_add += 1
                code_buffer_index += 1

                code_buffer[code_buffer_index] = (((match_position >> 4) & 0xF0) | (self.context["match_length"] - self.context["match_threshold"])) & 0xFF
                code_add += 1
                code_buffer_index += 1
            ...
            if last_match_length > 0:
                ii = 0
                while ii < last_match_length and i < uncompressed_size:
